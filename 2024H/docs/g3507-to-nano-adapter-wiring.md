...
| 外接启动按钮，备用 | D4 | 7 | H1-7 | PB17 | 备用；当前代码改用 C07A 板载 SW1/PB8 |
...
当前启动按钮代码已改为使用 C07A 板载 SW1：

- `include/platform/c07a_pinmap.h`：启动按钮信号为 `PB8`，低电平有效。
...
当前 `src/platform/platform_mspm0.c` 里的其他 `platform_hw_*` 仍是 weak 空实现，需要按这个表做真实 GPIO、PWM、I2C 和中断配置。
